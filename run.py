...
# 部署上线gitlab地址
feReleaseGit = 'https://github.com/xmllein/fis3-static-demo-release.git'
...
def releaseOnline():
    ...
    os.chdir(currPath)
    exeCmd('rm -rf ' + os.path.join(feRelease + 'fis3-static-demo-release', "*"))

    # 拷贝到gitlab目录
    cmd = 'scp -r ' + os.path.join(bakTmp, '', '*') + ' ' + os.path.join(feRelease + 'fis3-static-demo-release', '')
    exeCmd(cmd)

    # 切到fe-release git push
    os.chdir(os.path.join(currPath, feRelease + '/fis3-static-demo-release', ''))
    exeCmd('git add .')
